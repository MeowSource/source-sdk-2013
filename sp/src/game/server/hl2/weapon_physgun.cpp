...
//
// Purpose: Server-side version of the Physgun, restores beam drawing.
// 
...
#include "player.h"
#include "gamerules.h"
#include "basehlcombatweapon.h" // Formerly #include "basecombatweapon.h"
...
#include "materialsystem/IMaterialVar.h"   // For IMaterialVar class (this is the correct header)
	...
	if (m_align)
	{
		...
		if (len > 0.08)
		{
			...
			if (rotRadius > 0.1)
			{
				...
				world = m_targetPosition;// + rotDest * (1-ratio); // Uncommented.
...
class CWeaponGravityGun : public CBaseHLCombatWeapon // Formerly CBaseCombatWeapon.
{
	DECLARE_DATADESC();

public:
	DECLARE_CLASS(CWeaponGravityGun, CBaseHLCombatWeapon); // Formerly CBaseCombatWeapon.
	...
	{
		EffectDestroy();
		return BaseClass::Holster(pSwitchingTo); // Added pSwitchingTo.
	}
...
SendPropFloat(SENDINFO(timer), 32, SPROP_NOSCALE),
...
DEFINE_FIELD(timer, FIELD_FLOAT),
...
void CWeaponGravityGun::EffectUpdate(void)
{
	...
	IMaterial* pMaterial = materials->FindMaterial("models/Weapons/V_physics_gun/redrotater", TEXTURE_GROUP_OTHER, true);
	if (!pMaterial)
	{
		Msg("Material not found: %s\n", "models/Weapons/V_physics_gun/redrotater");
		return;
	}

	// Modify the $color parameter to adjust the weapon's base color
	IMaterialVar* pColorVar = pMaterial->FindVar("$color", NULL);

	if (pColorVar)
	{
		// Set the $color parameter with the new color (RGB values)
		pColorVar->SetVecValue(colour.x, colour.y, colour.z);
	}
	else
	{
		Msg("Material does not have a $color parameter\n");
	}

	// Optionally, modify the self-illumination tint (for glow effects, etc.)
	IMaterialVar* pSelfIllumVar = pMaterial->FindVar("$selfillumtint", NULL);

	if (pSelfIllumVar)
	{
		// Set the $selfillumtint to the new color if you want the glow effect
		pSelfIllumVar->SetVecValue(colour.x, colour.y, colour.z);
	}

	// Force the material to update and apply the changes
	pMaterial->IncrementReferenceCount();  // Ensure the material is not unloaded prematurely
	...
	if (pObject)
	{
		...
		if (tr.fraction == 1)
		{
			...
			float distance2 = VectorNormalize(dir);
...
void CWeaponGravityGun::SoundUpdate(void)
{
	...
	case SS_LOCKEDON:
	{
		...
		// -------------------------------------------------------------------
		// Addition to prevent a game crash when certain objects are grabbed.
		// Not covered in a video (yet!), can kill certain NPCs i.e. Manhacks.
		// -------------------------------------------------------------------
...
// This is already defined elsewhere, so we can comment this whole thing out.
